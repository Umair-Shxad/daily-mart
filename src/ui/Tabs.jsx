...
function Tabs({ tabs }) {
  ...
  const ActiveComponent = tabs.find((tab) => tab.type === activeTab)?.content;
    ...
    <div className="px-10 my-10">
      <ul className="flex justify-center gap-6 text-sm mb-10">
        {tabs.map(({ title, type }) => (
          <li key={type}>
            <button
              className={`uppercase cursor-pointer px-4 pb-2 ${activeTab === type ? "border-b-2 border-dashed" : "opacity-60"}`}
...
export { Description, Table, Reviews };
